...
class ClientWrapper(Wrapper):
    ...
    def get_client(self, workspace_id: str, client_id: str) -> Client:
        url = self._url(workspace_id, client_id)
        return self.get_one(url, Client)

    def create_client(self, client: Client) -> Client:
        url = self._url(client.workspace_id)
    ...
    def update_client(self, client: Client) -> Client:
        url = self._url(client.workspace_id, client.id_)
        return self.update_one(url, client, Client)

    def _url(self, workspace_id: str, client_id: str = None) -> str:
