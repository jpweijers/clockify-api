...
class TestTags(ClockifyTestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        for name in tag_names:
            ...
            cls.session.tag.create_tag(tag)
    ...
    def test_get_list_of_tags(self):
        tags = self.session.tag.get_tags(self.WORKSPACE)
        for tag in tags:
            self.assertIsInstance(tag, Tag)

    def test_get_list_of_tags_fail(self):
        self.assertRaises(HTTPError, self.session.tag.get_tags, "fake workspace id")

    def test_get_tags_with_params(self):
        params = TagGetParams(archived=False)
        tags = self.session.tag.get_tags(self.WORKSPACE, params)
        for tag in tags:
            self.assertIsInstance(tag, Tag)

    def test_get_tag_by_id(self):
        tag = Tag(name=f"Test Tag #{randint(0, 99999)}", workspace_id=self.WORKSPACE)
        tag = self.session.tag.create_tag(tag)
        search_tag = self.session.tag.get_tag(self.WORKSPACE, tag.id_)
        self.assertIsInstance(search_tag, Tag)
        self.assertEqual(tag, search_tag)

    def test_get_tag_by_id_fail(self):
        self.assertRaises(
            HTTPError, self.session.tag.get_tag, self.WORKSPACE, "fake tag id"
        )
    ...
    def test_update_tag(self):
        ...
        tag = self.session.tag.create_tag(tag)
        tag.name = "pizza"
        updated_tag = self.session.tag.update_tag(tag)
        self.assertIsInstance(updated_tag, Tag)
        self.assertEqual(updated_tag.name, "pizza")

    def test_create_tag(self):
        tag = Tag(name=f"Test Tag #{randint(0, 99999)}", workspace_id=self.WORKSPACE)
        created_tag = self.session.tag.create_tag(tag)
    ...
    def test_delete_tag(self):
        ...
        tag = self.session.tag.create_tag(tag)
        tag.archived = True
        archived_tag = self.session.tag.update_tag(tag)
        deleted_tag = self.session.tag.delete_tag(
